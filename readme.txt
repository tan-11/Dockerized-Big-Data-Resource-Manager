...
PowerDockerLab::Containerized Big Data Learning Environment (PaaS)


1. PROJECT OVERVIEW
-------------------
PowerDockerLab is a lightweight Platform-as-a-Service (PaaS) designed to automate
the deployment of Big Data learning environments. Unlike traditional Virtual 
Machines (VMs) which are resource-heavy, this platform utilizes Docker containers
to provide students with instant, isolated access to Hadoop and Spark clusters.

The system features a "Self-Service" web portal that allows users to provision
environments, manages persistent storage so data survives restarts, and enforces
strict resource governance (CPU/RAM quotas) to ensure fair multi-tenant usage.
...
Step 3: Install Python Libraries
    	...
    	(Key libraries: Flask, Docker SDK for Python, Paramiko, Psutil)
...
2.  Access the Web Portal:
    Open your web browser and navigate to: http://localhost:5000(user site) & http://localhost:5000(admin site)
...
6.  Connect via Terminal: copy and paste the SSH command.
...
├── user_data/             # Persistent storage mount points for users (exist after fisrt user container creation)
